...
#                 ymin, ymax = min(mod_patch_coord_list[cnt][:,1]), max(mod_patch_coord_list[cnt][:,1])
                ...
                for n1_idx, n2_idx in patch_edge:
                    ...
                    cv2.circle(img, (p1[1], p1[0]), 3, (0,255,0), -1)
                    cv2.circle(img, (p2[1], p2[0]), 3, (0,255,0), -1)
                    cv2.line(img, (p1[1], p1[0]), (p2[1], p2[0]), (255,255,255), 2)
#                     for x, y, _ in patch_coord:
#                         xx, yy = (x*patch_size[0]).astype('int32'), (y*patch_size[0]).astype('int32')
#                         cv2.circle(img, (xx, yy), 3, (0,255,0), -1)
                cv2.imwrite(os.path.join(save_path, '%d_shp.png'%image_id), img)
                cv2.imwrite(os.path.join(save_path, '%d_data.png'%image_id), patch)
                cv2.imwrite(os.path.join(save_path, '%d_seg.png'%image_id), patch_seg)
...
if __name__ == "__main__":
    root_dir = "/data/weiweidu/criticalmaas_data/training_fault_line"

    image_id = 1
    train_path = './data/darpa/fault_lines/check/'
...
#                 if 'scarp_line' in f and 'png' == fmt and name in f:
            if 'png' == fmt and 'fault_line' not in f :
                raw_files.append(os.path.join(root, f))
                seg_files.append(os.path.join(root, f[:-4]+'_fault_line.png'))
                vtk_files.append(os.path.join(root, f[:-4]+'_fault_line.p'))

    print(len(seg_files), len(raw_files), len(mask_files), len(vtk_files))   

#     for ind in range(len(raw_files)):
    for ind in range(1):
        print(ind, raw_files[ind], seg_files[ind], vtk_files[ind])
        try:
            sat_img = imageio.imread(raw_files[ind])
        except:
            sat_img = imageio.imread(raw_files[ind][:-4]+".jpg")
        if mask_files == []:
