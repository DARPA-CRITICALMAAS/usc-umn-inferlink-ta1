...
import json
import subprocess
from gpkg_writer_geo_coord import read_georef_output
...
def get_gcp_from_geojson(georef_json):
    gcp_pairs = []
    for item in georef_json['gcps']:
        temp_pair = [item['px_geom'][0], item['px_geom'][1], item['map_geom'][0], item['map_geom'][1]]
        gcp_pairs.append(temp_pair)
    return gcp_pairs
...
#     command = f"gdal_translate -of GTiff -gcp {gcp_pair0[0]} {gcp_pair0[1]} {gcp_pair0[2]} {gcp_pair0[3]} -gcp {gcp_pair1[0]} {gcp_pair1[1]} {gcp_pair1[2]} {gcp_pair1[3]} -gcp {gcp_pair2[0]} {gcp_pair2[1]} {gcp_pair2[2]} {gcp_pair3[3]} -gcp {gcp_pair3[0]} {gcp_pair3[1]} {gcp_pair3[2]} {gcp_pair3[3]} \'{input_tif_path}\' \'{output_temp_tif_path}\'"
    ...
    for img_x, img_y, geo_x, geo_y in gcp:
        command += f' -gcp {img_x} {img_y} {geo_x} {geo_y}'
...
def run_georeference_map(map_name, input_map_dir, gcp_path, geo_tif_output_dir, temp_dir='./temp'):
    ...
    geojson_ref = read_georef_output(gcp_path)

    gcp_pairs = get_gcp_from_geojson(geojson_ref)
