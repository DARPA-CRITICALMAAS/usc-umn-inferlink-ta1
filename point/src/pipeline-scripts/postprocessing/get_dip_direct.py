    ...
    if os.path.isfile(strike_geojson):
        for each_pt in strike_list:
            ...
            if os.path.isfile(od_geojson):
                ...
                for stk_feature in stk_output["features"]:
                # dip direction value from each instance
                    dd = stk_feature["properties"]["dip_direction"]
                    cnt_pt_st = Point(stk_feature["geometry"]["coordinates"])
                    radius = 20
                    ...
                    for pt_feature in pnt_output["features"]:
                        cnt_pt = Point(pt_feature["geometry"]["coordinates"])        
                        if st_circle.contains(cnt_pt):
                            dist = cnt_pt_st.distance(cnt_pt)
                            if min_dist > dist:
                                min_dist = dist
                                min_id = pt_feature["properties"]["id"]

                    for pt_feature in pnt_output["features"]:
                        if min_dist != 10000 and (int)(pt_feature["properties"]["id"]) == (int)(min_id):
                            pt_feature["properties"]["dip_direction"] = dd
