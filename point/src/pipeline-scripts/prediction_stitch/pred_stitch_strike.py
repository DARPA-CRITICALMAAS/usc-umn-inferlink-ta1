...
def stitch_to_each_strike(map_name, crop_dir_path,pred_root,stitch_root, map_sheets_dir ,save_raster, cmp_eval, pnt_pair_per_map ): 
    ...
    if len(file_list) != 0:
        ...
        for index, line_data in map_df.iterrows():
            ...
            features_per_symbol[sym_type].append(Feature(geometry = point, properties={'type': sym_type, "id": len(features_per_symbol[sym_type]), "score": score, "r_bbox": bbox ,"dip" : 0 ,"dip_direction" : dip_direction, "provenance": "modelled" }))
