...
def find_correspondences(image_path1: str, image_path2: str, num_pairs: int = 10, load_size: int = 224, layer: int = 9,
    ...
    if normalized.shape[0] == 0:
        #not enough best buddies pairs were found-> return empties
        return [], [], [], [], []
    ...
    #sort topk by similarities and also indices to show by sim
    combined_lists = list(zip(bb_topk_sims, bb_indices_to_show))
    sorted_combined_lists = sorted(combined_lists, key=lambda x: x[0], reverse=True)
    bb_topk_sims = [t[0] for t in sorted_combined_lists]
    bb_indices_to_show = [int(t[1]) for t in sorted_combined_lists]
    ...
    return bb_topk_sims, points1, points2, image1_pil, image2_pil

def find_correspondences_2(image_path1: str, image_path2: str, num_pairs: int = 10, load_size: int = 224, layer: int = 9,
                         facet: str = 'key', bin: bool = True, thresh: float = 0.05, model_type: str = 'dino_vits8',
                         stride: int = 4) -> Tuple[List[Tuple[float, float]], List[Tuple[float, float]],
                                                                              Image.Image, Image.Image]:
    """
    ELIMINATING K MEANS SO THAT MATCHING POINTS ARE NOT WELL DISTRIBUTED
    finding point correspondences between two images.
    :param image_path1: path to the first image.
    :param image_path2: path to the second image.
    :param num_pairs: number of outputted corresponding pairs.
