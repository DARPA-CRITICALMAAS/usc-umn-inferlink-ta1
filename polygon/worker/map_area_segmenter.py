...
def cropping_worker(input_dir, input_img, output_path, intermediate_path):
    input_path = os.path.join(input_dir, input_img)
    print('Working on input image:', input_path)
    ...
    out_file_path0=intermediate_path+'/'+str(input_img)+'_cropping_1_1.tif'
    cv2.imwrite(out_file_path0, selected_map_for_examination)

    print_bgr = cv2.cvtColor(crop_rgb2, cv2.COLOR_RGB2BGR)
    out_file_path0=intermediate_path+'/'+str(input_img)+'_cropping_1_2.tif'
    ...
    candidate_filing = intermediate_path+'/'+str(input_img)+'_cropping_1_1.tif'
    ...
    if arg_sort.shape[0] > 0:
        ...
        selected_index = arg_sort[0]+1 # 1
    selected_map_for_examination = np.zeros((labeled00.shape[0],labeled00.shape[1],1),dtype=np.uint8)
    selected_map_for_examination[labeled00 == selected_index] = 255

    out_file_path0 = intermediate_path+'/'+str(input_img)+'_cropping_2_1.tif'
    cv2.imwrite(out_file_path0, selected_map_for_examination)

    out_file_path0 = output_path
    cv2.imwrite(out_file_path0, selected_map_for_examination)
    ...
    out_file_path0=intermediate_path+'/cropping_2_2.tif'
