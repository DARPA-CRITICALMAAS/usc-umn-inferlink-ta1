...
def polygon_output_handler():
    ...
    if os.path.isfile(path_to_legend_solution) == False:
        ...
        for fname in os.listdir(dir_to_raster_polygon):    # change directory as needed
            if os.path.isfile(os.path.join(dir_to_raster_polygon, fname)):
                ...
                if '_predict.tif' in fname and map_name.replace('.tif', '_') in fname:
    ...
    #    if os.path.isfile(os.path.join(dir_to_raster_polygon, fname)):
    #        if '_predict.tif' in fname and map_name.replace('.tif', '_') in fname:
    #            shutil.copyfile(os.path.join(dir_to_raster_polygon, fname), os.path.join(dir_to_raster_output, map_name, fname.replace('_predict.tif', '_predict.tif')))
    # we already made sure that the naming convention is the same
    ...
    for fname in os.listdir(dir_to_raster_polygon):
        if os.path.isfile(os.path.join(dir_to_raster_polygon, fname)):
            #print(os.path.join(dir_to_raster_polygon, fname), map_name.replace('.tif', '_'))
            if '_predict.tif' in fname and map_name.replace('.tif', '_') in fname:
                path_to_extraction_geojson = os.path.join(dir_to_integrated_output, map_name, fname.replace('_predict.tif', '_predict.geojson'))
                path_to_extraction_tif = os.path.join(dir_to_raster_output, map_name, fname.replace('_predict.tif', '_predict.tif'))
