...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, training_map: np, auxiliary_dict: dict, scale: float = 1.0, mask_suffix: str = '', mask2_suffix: str = ''):
        self.images_dir = Path(images_dir)
        self.mask_dir = Path(mask_dir)
        assert 0 < scale <= 1, 'Scale must be between 0 and 1'
        self.scale = scale
        self.mask_suffix = mask_suffix
        self.mask2_suffix = mask2_suffix
        self.auxiliary_dict = auxiliary_dict.copy()

        self.ids = [splitext(file)[0] for file in listdir(images_dir) if isfile(join(images_dir, file)) and not file.startswith('.') and (('_'.join(splitext(file)[0].split('_')[:-2])) in training_map)]
        if not self.ids:
    ...
    def __getitem__(self, idx):
        ...
        #mask2_file = list(self.mask_dir.glob(name + self.mask2_suffix + '.*'))
        mask_file = list(self.mask_dir.glob(name + '_mask.*'))
        mask2_file = list(self.mask_dir.glob(name + '_mask2.*'))
        img_file = list(self.images_dir.glob(name + '.*'))

        assert len(img_file) == 1, f'Either no image or multiple images found for the ID {name}: {img_file}'
        assert len(mask_file) == 1, f'Either no mask or multiple masks found for the ID {name}: {mask_file}'
        assert len(mask2_file) == 1, f'Either no mask or multiple masks found for the ID {name}: {mask2_file}'
        mask = load_image(mask_file[0])
        mask2 = load_image(mask2_file[0])
        ...
        img_sup_6 = load_image(join(self.images_dir, 'sup', name+'_sup_6.png'))
        img_sup_7 = load_image(join(self.images_dir, 'sup', name+'_sup_7.png'))
        img_sup_8 = load_image(join(self.images_dir, 'sup', name+'_sup_8.png'))

        assert img.size == mask.size, \
            f'Image and mask {name} should be the same size, but are {img.size} and {mask.size}'
        assert img.size == mask2.size, \
            f'Image and mask2 {name} should be the same size, but are {img.size} and {mask2.size}'
        
        img = self.preprocess(self.mask_values, img, self.scale, is_mask=False)
        mask = self.preprocess(self.mask_values, mask, self.scale, is_mask=True)
        mask2 = self.preprocess(self.mask_values, mask2, self.scale, is_mask=True)
        ...
        img_sup_6 = self.preprocess(self.mask_values, img_sup_6, self.scale, is_mask=False)
        img_sup_7 = self.preprocess(self.mask_values, img_sup_7, self.scale, is_mask=False)
        img_sup_8 = self.preprocess(self.mask_values, img_sup_8, self.scale, is_mask=False)

        img_combined = np.zeros((10, img.shape[1], img.shape[2]), dtype='uint8').astype(float)
        img_combined[0] = img
        ...
        return {
            ...
            'auxiliary_1': self.auxiliary_dict[str(name)][0],
            'auxiliary_2': self.auxiliary_dict[str(name)][1]
        }


class CarvanaDataset(BasicDataset):
    def __init__(self, images_dir, mask_dir, training_map, auxiliary_dict, scale=1):
        super().__init__(images_dir, mask_dir, training_map,auxiliary_dict, scale, mask_suffix='_mask', mask2_suffix='_mask2')