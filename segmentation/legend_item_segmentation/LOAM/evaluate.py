...
def evaluate(net, dataloader, device, amp):
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
            ...
            image, mask_true, mask_true_sup, auxiliary_info_1, auxiliary_info_2 = batch['image'], batch['mask'], batch['mask2'], batch['auxiliary_1'], batch['auxiliary_2']
            ...
            # predict the mask
            mask_pred, mask_pred_sup = net(image, auxiliary_info_1, auxiliary_info_2)

            if net.n_classes == 1:
                assert mask_true.min() >= 0 and mask_true.max() <= 1, 'True mask indices should be in [0, 1]'
                mask_pred = (F.sigmoid(mask_pred) > 0.5).float()
                mask_pred_sup = (F.sigmoid(mask_pred_sup) > 0.5).float()
                # compute the Dice score
                dice_score += dice_coeff(mask_pred, mask_true, reduce_batch_first=False)
                ...
                mask_pred_sup = F.one_hot(mask_pred_sup.argmax(dim=1), net.n_classes).permute(0, 3, 1, 2).float()

                # compute the Dice score, ignoring background
                dice_score += multiclass_dice_coeff(mask_pred[:, 1:], mask_true[:, 1:], reduce_batch_first=False)
                dice_score += multiclass_dice_coeff(mask_pred_sup[:, 1:], mask_true_sup[:, 1:], reduce_batch_first=False)
            dice_score = dice_score / 2.0
